...
- Assets and mod metadata are in `src/main/resources/` (`fabric.mod.json`, `ludwigmod.mixins.json`, language files, textures, shaders).
- `build/` is generated output; do not commit manual edits there.

## Build, Test, and Development Commands
- `./gradlew build`: main CI command; compiles, remaps, and packages the mod jar.
...
- Java 17 is required; keep code compatible with the versions in `gradle.properties`.
...
- Ensure GitHub Actions `build` passes on Java 17 before requesting review.
