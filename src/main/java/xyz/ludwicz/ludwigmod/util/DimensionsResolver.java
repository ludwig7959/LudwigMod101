...
import xyz.ludwicz.ludwigmod.borderless.BorderlessFullScreen;
import xyz.ludwicz.ludwigmod.borderless.BorderlessFullScreen.CustomWindowDimensions;
...
public class DimensionsResolver {
    ...
    public boolean resolve(Window window, MonitorTracker tracker) {
        if (BorderlessFullScreen.customWindowDimensions != null &&
                BorderlessFullScreen.customWindowDimensions.enabled &&
                !BorderlessFullScreen.customWindowDimensions.useMonitorCoordinates) {
            x = 0;
            y = 0;
            width = 0;
            height = 0;
        } else if (BorderlessFullScreen.forceWindowMonitor < 0) {
            ...
            long monitorHandle;
            if (BorderlessFullScreen.forceWindowMonitor >= monitors.limit()) {
                LOGGER.warn("Monitor " + BorderlessFullScreen.forceWindowMonitor + " is greater than list size " + monitors.limit() + ", using monitor 0");
                monitorHandle = monitors.get(0);
            } else {
                monitorHandle = monitors.get(BorderlessFullScreen.forceWindowMonitor);
            }
        ...
        if (BorderlessFullScreen.customWindowDimensions != null) {
            CustomWindowDimensions dims = BorderlessFullScreen.customWindowDimensions;
            if (dims.enabled) {
